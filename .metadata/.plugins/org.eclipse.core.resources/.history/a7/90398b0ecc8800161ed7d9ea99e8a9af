...
public class uniformLbpMaintrain {
...
//     File file = fc.getSelectedFile();
		...
		for (int i = 0; i < height-2; i++) {
			for (int j = 0; j < width-2; j++) {
				...
				int count=0;
				for (int i1 = 0; i1 < binaryarray.length-1; i1++) {
					if(binaryarray[i1]!=binaryarray[i1+1]){
						count++;
					}
		...
		Writer writer = new BufferedWriter(new OutputStreamWriter(
			new FileOutputStream("D:/image/uniformtrain.txt", true), "UTF-8"));
		String s="3 ";
